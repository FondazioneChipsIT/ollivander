...
**Silence is not a stall.** Before concluding that a run is stuck, measure progress: the transcript's modification time, the `[TB_TIME]` heartbeat, the simulated time advancing. Firmware phases have long quiet stretches, and healthy runs have been killed on the assumption that quiet meant hung. Two instruments narrow the question on an offload run (testbench guide): the last `[TEST_PROGRESS]` phase code says which phase a quiet run is in, and the `+progress` activity picture says what the interconnect is doing there - on a mesh a tile beating high while its neighbours sit at zero, on a crossbar one port at full rate while the others are idle, is a poll that will not end (something it waits for is not coming); everything at zero is a real stall. Before simulating, `generated/doc/noc_placement_report.md` says where XY routing funnels the traffic (static router load per network) and how far each workload path travels; a hot column in the run-time grid that the static map also predicts is placement, not a wedge. The VIP's load agents also print an expected duration at the start of every streamed image load and one line per tenth of it, so a JTAG preload of several milliseconds is no longer a silent stretch. When a collective phase is the one that stalls, `+collective_trace` shows which tiles injected, which routers reduced and where the result landed (`[COLL_TRACE]`, printed from inside the tiles): a missing injection, a reduction that never produces its landing, or a landing at the wrong slot each read differently. A wrong reduction result no longer looks like either: since the payload waits for a landing to leave its sentinel rather than for an exact value, the host reports `[COLLECTIVE] <target> collect=<value> (expected <value>) ...` as soon as it lands, instead of running to the watchdog.
...
**A `NoNarrowMgrPortBResponse` (or `...RResponse`) assertion storming from a slave-only tile - typically `i_tile_0_0` - is a response whose destination was never looked up.** A chimney builds a B/R flit's destination from the id it stored at request time; a response coming back from the isle with a DIFFERENT AXI id misses that lookup, the destination defaults to zero, and the flit lands on tile (0,0), whose chimney has no manager port and can never accept it - so the assertion fires every cycle from then on, and the requester hangs. Look at the isle's type parameters before anything else: a `parameter type` left at its `logic` default (a channel struct the tile did not override) narrows the whole transaction to one bit through any `axi_cut` inside the isle. The first mesh simulation of the mailbox isle failed exactly so. `[TB_IRQ]` lines tell whether the write ever reached the mailbox: no change of `intr_<mailbox>_snd_irq_o` means it did not.

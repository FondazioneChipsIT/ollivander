...
*   **Macro export: harmonize on the isle contract, symmetric boundary translation (decided with the user, 2026-08-11).** Today the two families diverge: crossbar macros keep GLOBAL addresses inside (`InstanceBaseAddr` emitted as `MACRO_BASE_ADDR + base`), NoC macros rebase to LOCAL at their border adapters - and neither path is exercised end-to-end by any test. The agreed plan, in order: (1) a **nested-access test** on both families first - the parent host writes and reads the nested macro's L2 through the macro window; it is written against the window contract, so it validates before and after the change, and would catch a rebase/decode mismatch that today hides exactly like the DECERR defect of 2026-08-10 did. (2) **One contract**: macro tops declare `InstanceBaseAddr`/`InstanceWindowSize` like any self-mapping isle (`MACRO_BASE_ADDR` renamed, the special case in `rtl_ir_builder.py` deleted; macro ARRAYS inherit the per-instance route for free). (3) **One implementation**: boundary translation on BOTH families - the NoC border adapters already subtract; the crossbar macro top gains the same remap stage on its exported slave ports (an adder on the AW/AR path, cuttable for ASIC timing) - and every identity parameter INSIDE a macro goes back to project-local, removing relocation arithmetic from the generator entirely. Suspicion to verify with the test: crux_isle's internal xbar rules may be local while its identity params were global, i.e. the crossbar macro was already broken for a nonzero base and nothing ever noticed. (4) **Outgoing-traffic semantics documented once for both families**: local addresses for a macro's own ranges, parent-global passthrough for external targets.

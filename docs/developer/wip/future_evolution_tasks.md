...
*   **Phase 2b, in progress (frozen 2026-08-07 late evening): the `"memory_mapped"` realization is IMPLEMENTED and compiled into crux, but the spatz bring-up has one open defect.** What landed: the `Offload*` contract on `spatz_cluster_isle` (entry via CLUSTER_BOOT_CONTROL, wake via the cluster CLINT, per-core `(value << 1) | 1` return slots in TCDM), per-contract eligibility in the schema (memory_mapped needs neither fetch_enable nor EOC), the memory_mapped legs in the offload templates and payload, and four real defects fixed on the way - the IP's bootrom baked its default config's absolute TCDM base into BOOTDATA (registry-patched to PC-relative arithmetic, which in turn required relocating the bootrom from absolute 'h1000 to cluster_base + 'h3_0000, since position-independent code cannot reach a disjoint block), the isle's PMA was carfield's fossil with no fetchable region (the cores never had a valid PC - first time code ever ran on them), and the host CLINT's mtip fed the cores X-at-boot / 1-at-rest (tied off; the offload model wakes through the internal CLINT only, like gwaihir). OPEN: with all irq inputs clean the cores still take a trap between the bootrom's `csrw mtvec` and its `wfi` (PC lands on `dummy_exception` by 10 us, then the state degrades to X; +acc probe evidence in the session of 2026-08-07). Next probes: the cluster-internal CLINT/peripheral outputs during the first microseconds, and the CSR ops themselves (mie is implemented in snitch.sv, but the bootrom also touches mstatus/mcause paths). crux parks spatz out of the test via `offload_targets: ["pulp_cluster"]` until this lands; drop that line to resume.

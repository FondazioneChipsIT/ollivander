...
*   **Phase 2b, CLOSED 2026-08-10: crux passes BOTH contracts in one firmware** (pulp_cluster control_wire + spatz_cluster memory_mapped, PASS with EOT under QuestaSim). The 2026-08-07 "CSR-setup trap" was never a trap: the cores marched linearly through accidental ALU ops because every instruction fetch returned the low words of its 64-byte ROM line - the bootrom returns whole 512-bit lines and ignores addr[5:0] BY DESIGN, and our isle runs the cluster DMA bus at the SoC's 64-bit width, so the rdata connection silently truncated (upstream runs that bus at 512 and never sees it). Repaired in patch_spatz.py (the ROM now latches addr[5:0] and shifts the line - correct for any reader width), where the PC-relative entry patches also moved once the set passed the three-inline limit. One convention learned on the way: the memory_mapped return slots carry (value << 1) | 1, so only 31 bits of value survive - the toy workload's whitening constant now keeps bit 31 clear (rtl_generator.py).
*   **Phase 2c, CLOSED for the mesh family 2026-08-11: noc, noc_subtile and noc_isle all PASS the offload on their full 16-cluster arrays in parallel** (one shared payload, per-instance windows, EOT ~1.5 ms simulated each). The multi-instance decision fell on the parallel launch; the behaviour is user documentation now (`docs/soc_configuration_guide.md` section 5.1: instance arrays, `payload_memory`, control-group ungating, the CIE/LLC-out cache calibrations). Three stacked reachability defects had to fall first, each masking the next: the clustergen wrapper ignored its identity ports (fixed via `cluster_base_expose` in the cfg patch), the SoC top tied them to `'0` (per-instance base/extent now wired by `rtl_ir_builder.py`), and the generated Cheshire external regions spanned a single `size_per_instance` — every access beyond instance 0 died in the host's internal DECERR slave, B response and all, leaving no trace on the network. The supers are being validated with the same machinery (super_crossbar replays the crux pair; super_noc drives its own cluster array and needed `LlcOutRegionStart/End` parametrized on `cheshire_isle` to keep its high-mapped cluster windows uncached).
...
### 3.9 Fold `L2BaseAddr`/`L2MemSize` into the Instance Identity Convention

The per-instance window parameters now have a documented, header-declared convention: `InstanceBaseAddr`/`InstanceWindowSize` (`docs/hw/subtile_standardization.md` section 2.6), filled by the generator whenever the header declares them, no name or type matching involved. The older `L2BaseAddr`/`L2MemSize` pair (section 2.5 there) predates it and still carries two blemishes the new route was designed without: the generator gates it on a component-name heuristic (`'l2' in c.name or system_config.is_l2_mem`, `rtl_ir_builder.py`) rather than on the header alone, and it duplicates the same base+index*stride computation. Migrating the memory isles to the generic pair (keeping the old names accepted for a deprecation window, or renaming outright while the user base is zero) would leave ONE instance-identity mechanism.

*   **Advantages**: one convention instead of two; removes the last component-name heuristic from the instantiation path; the macro-mode `MACRO_BASE_ADDR` special case of L2BaseAddr gets re-examined against the border-adapter rebase rule the new pair already follows.
*   **Difficulties & Mitigation Strategies**: the L2 wrappers consume `L2BaseAddr` in their internal rules, so the rename touches `l2_isle`/`sram_isle` and every example map at once - mechanical but broad; do it in a quiet window with a full `TEST_SIM=1` suite on both families. The macro-mode question needs its own check on super_crossbar (the nested Mesh macro's L2 tiles) before changing anything there. Same occasion: carry the declared parameter TYPE through the SV parser, so the tile wrapper stops inferring types from normalized default values - today that infers `int unsigned` and caps instance-identity values at 4 GiB (documented bound in the standardization doc).

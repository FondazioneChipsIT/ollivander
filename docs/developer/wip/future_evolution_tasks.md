...
Re-measured on Verilator 5.050 on 2026-08-05 (probes on `crux`, after the section 3.5 landing removed the `pulp_cluster_wrap` shell). Two whole layers turned out to be flow gaps, not IP walls, and are fixed generically; what remains is enumerated to the line.

*   **Flow gap, includes**: vlog resolves `` `include `` relative to the including file, Verilator only against cwd and `+incdir` — 29 of the first probe's errors (all of softex, one in vendored ibex) were exactly this. `--relative-includes` replicates the vlog semantics and sits in the template's `VERILATOR_SIM_FLAGS`; the `noc` flow was re-validated end-to-end with it on 2026-08-06 (UART + EOT, same pass criterion).
*   **Flow gap, component defines**: the `DEFINE:` mechanism injects (e.g.) `FEATURE_ICACHE_STAT` into every `vlog` call of the vsim script but never reaches the Verilator invocation; hier-icache's statistics counters then kill the parse. The template must append the `global_defines` list to the Verilator flags exactly as it patches the vsim script.
*   **Verilator's constant evaluator cannot read unpacked-struct constants** (`SimulateVisitor: unknown node`, 5.050). It killed the isle's copy-then-override constant function (replaced with a full `'{...}` literal, which elaborates — the spatz isle proved the form) but persists inside the IP: `Cfg.NumCores` as a generate-for bound and `Cfg.HwpeCfg` passed down to `hwpe_subsystem` still fail to fold, because `pulp_cluster_cfg_t` and `hwpe_subsystem_cfg_t` are unpacked. The candidate fix is the proven packed-ification playbook (catalogue patch `typedef struct` → `typedef struct packed` — every member is packable); upstream-worthy on both sides (Verilator limitation, pulp_cluster typedef).
*   **axi version skew, two sites**: `xbar_cfg_t` literals written before this axi fork added `NoMulticastRules`/`NoMulticastPorts` no longer initialize every member — `secure_subsystem_asynch_synth_wrap_astral.sv:508` and pulp_cluster's `cluster_bus_wrap.sv:148`. One `default: '0` per literal.
*   **Remaining single sites, unmeasured depth**: OpenTitan's `OTClusterCfg` defparam (same unpacked-struct family), two errors in OpenTitan's `crypto_sram_wrap/idma_wrap.sv`, and spatz's `tcdm_test.sv` (a bind-style test module, candidate for `VERILATOR_FLIST_EXCLUDE`). Each visible only since the layer above it cleared; more may follow — the onion pattern of the mesh campaign.

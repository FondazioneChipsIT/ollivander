...
### 2.2 Cluster Offload Tests: Reference Analysis and the Generic Shape

Both reference designs were read on 2026-08-07 before starting this work; they embody two different CLUSTER BOOT CONTRACTS — a property of the instantiated IP, not of the topology (crux itself carries both kinds: pulp_cluster and the snitch-derived spatz).

*   **astral, `pulpd_offloader_blocking.c` (60 lines + ~40 of helpers)** — the control-wire contract of pulp_cluster: the host embeds the cluster binary in its own ELF (`payload.h`, an array plus `load_binary()`), copies it to L2, `fencei()` (the L2 is WB-cached: without the flush the cluster reads stale memory), writes the PER-CORE boot-address registers through the cluster's slave port AT RUNTIME (which removes the static `Cfg.BootAddr` fix of section 3.5 from this task's critical path — it stays right for standalone boot only), raises boot/fetch enable in the soc-control registers, polls the EOC register, reads the return register. Single host ELF, no testbench cooperation, silicon-identical.
*   **gwaihir, `simple_offload.c` (54 lines)** — the memory-mapped contract of snitch: the cluster binary is preloaded EXTERNALLY (their TB, via JTAG/slink; for us a `$readmemh` does it with machinery we already have), the host writes entry point and return-array address into the cluster's scratch registers (a snitch bootrom convention), wakes every core through the cluster CLINT, and polls a return array that must live in a NON-CACHED region (their comment says so explicitly).

**The generic shape, in Ollivander's own style**: a five-phase framework (load payload → configure entry → start → wait → collect), universal across IPs, with the per-IP realization DECLARED BY THE ISLE — the same pattern the `Preload*` localparams already use to describe a wrapper's memory layout to the testbench generator, extended to a boot contract for the firmware generator. The host-side helpers are then GENERATED from `system_config` plus that contract, which neither reference can do (astral hand-writes `pulp_cluster_start()` against hand-written registers; our boot/fetch/eoc registers are generated, so the helpers derive from the same source and cannot drift). The payload-embedding machinery (elf-to-header) is generic by construction. Two hazards imported from the references as framework rules: `fencei()` after every payload load, and the return region placed uncached.

**How the contract is exposed — decided 2026-08-07: `Offload*` localparams, following the `Preload*` precedent exactly.** The contract's substance is parameter-derived quantities (boot-address register offset and per-core stride, scratch offsets, a wakeup mask like `(1 << NumCores) - 1`), and a localparam can *derive* them at elaboration, is *type-checked* by the compiler (a typo is an elaboration error, where a comment pragma fails silently), and is *visible* hierarchically in debug; the contract kind itself is one more `localparam string OffloadContract` rather than a lone pragma that would split the mechanism. Parsed by the same `get_isle_info` that extracts `Preload*`. Two boundaries hold the design honest: the SYSTEM half of the contract (which boot/fetch/eoc registers the SoC generates) already lives in `system_config` and stays there — the isle declares only what the YAML cannot know, the IP-internal offsets and conventions; and after this week, header localparams are not free — the contract stays on scalars and strings (never `localparam type`, which evicts a wrapper from hierarchical verilation), with a one-off hier_block probe, favourable precedent being the `Preload*` strings already in the L2 tile's header.

...
*   **Phase 2c, the mesh examples.** The snitch_cluster bootrom is PC-relative out of the box (entry read from `scratch1` at a fixed offset from the bootrom itself), so the two spatz-specific traps above do not apply; the contract goes on `cluster_subtile` with the scratch1/CLINT offsets of the snitch peripheral map. Multi-instance targeting (mesh clusters are component ARRAYS) still needs a design decision: per-instance payloads and slots, or one instance as the representative target.
...
### 3.9 Consuming spatz Through Its Own Meta-Generation (clustergen)

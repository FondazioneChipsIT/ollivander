...
*   **`OffloadCollectOffs`** / **`OffloadBarrierOffs`** (`int unsigned`, optional, memory_mapped only): offsets of one 32-bit collect slot and one 32-bit barrier slot in the IP-local memory, above the return slots. Declaring them makes the component eligible for the collective (narrow-reduction) test when it is the NoC's multicast group: the generated tile stamps writes to instance 0's two slots with `IntAdd` and `LsbAnd` plus the group mask, every instance's core 0 writes there, the network merges the payloads, and the host verifies the reduced sum against a generator-derived expectation. The destination lives inside the group's own region by necessity: a reduction's member mask converts to coordinates only through the SAM rule of its destination.

...
*   **Purpose**: A project-specific wrapper that aggregates multiple `olli_rstgen` instances, one for each *managed* clock domain of the SoC. It takes the global Power-On Reset (POR) and an array of software-controlled resets, and outputs the final synchronized resets for all managed domains.
*   **Managed domains**: a clock domain is *managed* when it is neither a real-time domain (free-running, never gated or software-reset) nor the domain of the host, which is served by its own dedicated root reset generator (`i_host_rstgen`). Only managed domains appear in the `DomainIdx_*` enumeration and in the `rsts_n` / `pwr_on_rsts_n` vectors.
*   **Conditional generation**: this module, and the two vectors it drives, are emitted **only when the SoC has at least one managed domain**. A SoC whose only non-real-time domain is the host's — a common shape for single-clock NoC designs, and the one used by the gwaihir reference — legitimately has no global reset tree: its blocks take the reset straight from the top-level `rst_ni`, and per-tile software resets come from the auto control groups instead. In that case no `<project_name>_rstgen.sv` is generated, it is not listed in the Bender manifest, and no `rsts_n` vector is declared.
*   **Parameters**:

...
Enabling `narrow_reduction` has consequences beyond the routers: every tile gains FlooNoC's integer offload ALU (`floo_alu_top`, 32-bit operations — the IP's current limit), and the multicast group's tiles gain a generated *collective stamper* between isle and chimney. No CPU store can drive per-transaction AXI user bits, so the stamper is how software issues a collective: writes into the generated windows are stamped with an opcode and the member mask, everything else passes through untouched, and the SoC-wide user width never changes. The member mask a window stamps is generated; the **opcode of the reduction windows is not**: it comes from the System Controller's `collective_ctrl` register, one 4-bit field per multicast-target component and phase — `<component>_coll_col_op` for the column windows, `<component>_coll_row_op` for the row (or single-phase) window — holding FlooNoC's `collect_op_e` (`7` IntAdd, `8` IntMul, `9` IntMinS, `10` IntMinU, `11` IntMaxS, `12` IntMaxU; reset `7`), so an application changes the reduction without regenerating hardware. The barrier and multicast windows keep their fixed operation. A field is written only between phases, once the previous reduction has drained: changing it while a reduction is in flight is the interleaving the phase ordering below forbids. The generated offload test is the first consumer: its second power cycle programs IntMaxS on the row window and expects the maximum of the column sums. The FP (wide) reduction set stays confined to the compute tiles, the only place an FPU backs the offload interface. On the wide channel there is no stamper at all: the only wide master is the cluster's DMA, and its `dmuser` instruction writes the collective fields into the AXI user of the transfers it issues, so op and mask are runtime values rather than generated windows. This requires the cluster to be generated with its wide collectives on (`enable_wide_collectives`, a registry patch on its configuration), which widens its wide user to FlooNoC's `{collective_mask, collective_op}`; the network's wide channel declares exactly that layout and no plain user field, so cluster and network connect with a plain struct copy, and the component states the layout in its contract (`OffloadWideUserLayout`, component standardization 8.1) so that the tile types its wide isolation cell and chimney accordingly and rebuilds inbound requests into the component's own type.
...
*   **Every windowed slot is aligned to the beat of its channel** - the narrow beat for the narrow slots, the 64-byte wide beat for the wide landing (a wide transfer at a sub-beat offset degrades into strobed narrow beats). The collective machinery consumes the beat at channel width (`LsbAnd` reduces data bit 0 of the whole beat, the integer ALU computes on the low word), so a 32-bit store at an offset not aligned to the beat lands in the high half and is silently reduced as garbage. Unaligned contract offsets are refused at generation.

The barrier and the multicast are network capabilities always present in the emission, so they need no channel declaration: a component that declares the corresponding contract slots is tested with them even when no reduction channel is enabled. The reduction pair is the one that also needs `narrow_reduction` declared. A tile therefore emits between one and four collective windows depending on what its group can carry.

Collective phases must not interleave toward one destination: two streams with different masks and the same destination can head-of-line block each other in a router's reduction join, with no diagnostic. Each phase must therefore be drained before the next begins, and the drain must be observable by every participant rather than by the issuer alone — which is what orders the phases the generated firmware emits. The **barrier** runs first because it drains itself: a member's store returns only once the whole group has written, so when any core resumes the phase is over for everyone. The **reduction** follows, internally serialized and closed by a read of the final sum (reads do not reduce, so every core can confirm the drain). The **wide reduction** sits between them: the DMA hart sends the instance's eight FP64 lanes into the reduction and observes the merged landing with a plain remote read before releasing the cluster, so its drain is visible to every participant. The **multicast** runs last because its drain is the hardest to observe — members see the value land locally while the issuer is still collecting the responses — and nothing follows it that could collide.

Every NoC generation also writes `generated/doc/noc_placement_report.md`: the grid, the hop distances of the paths the generated workload uses (host to boot memory, host to each offload target, each target to its nearest memory tile), the static router load XY routing implies (the design-time twin of the testbench's `[TB_ACTIVITY]` grid), each collective group as the generator sees it (box, masks, phases, column heads, alias and slots, windows) from the same code the tile uses, a per-tile inventory, and the all-pairs distances folded at the end. Hop counts, never latencies. Which example exercises which mechanism is tabled in `soc_cfg_examples/README.md`. The generated offload test exercises the collectives wherever the hardware can carry them - the phase needs no switch of its own to appear. It has one to DISAPPEAR: `software_stack.test_app.collective_test: false` silences the firmware phase while leaving the emission untouched, for debugging or for a project that wants the hardware without the extra simulated milliseconds. The default is deliberately opt-OUT: collectives that ship enabled and never exercised are how this feature spent months looking healthy while being broken, so a project that skips the test states it in its own description. The console is quiet by default: the generated test prints the greeting, one final `[OFFLOAD] All targets passed.` line and the end-of-test byte, and sends every intermediate verdict as a 5 µs phase code the testbench renders with its time (`[TEST_PROGRESS] <time>  host: <target>: offload PASS`), because at the simulation-fast baud rate a printed line costs about 200 µs of simulated time on the host's critical path - half of a short run went into console output. `software_stack.test_app.verbose: true` restores the full per-cycle report on the UART; failure diagnostics are printed in both modes.
...
**Clock domains on a mesh.** Every non-real-time domain declared here exists on the NoC topology exactly as on the crossbar: its clock wire, mux and divider, its `<domain>_clk_en` / `<domain>_rst` / `<domain>_clk_div_value` registers and its place in the reset tree. A component placed in a domain other than the host's gets a tile with **two clocks**: the domain clock drives the isle (gated per tile by its control group, its reset release synchronised to that clock), while the router, the chimney and every network-side cell stay on the network clock with the network's power-on reset - a software reset of the domain never touches the mesh. The crossing depends on what the isle publishes: an isle with **asynchronous AXI** (its own CDC half, e.g. `pulp_cluster_isle`) is closed by the tile's bridge, which now has its two halves on the two clocks; an isle with **synchronous** req/rsp ports gets a full `axi_cdc` per data path inside the tile (`i_dom_cdc_<net>_mst` / `_slv`, isle types on the domain side, network types on the network side, the collective wide user carried whole), and `sync_domain` is not required to match on a mesh. Only a component taking the wide reduction offload is still refused outside the host's domain: that interface runs on the network clock and has no crossing. `noc_isle` runs its PULP array in a `pulp` domain and its Snitch box in a `cluster` domain, both at `default_div: 2`. The dividers of every domain are reset by the SoC's root power-on reset, not the domain's: the domain's reset is synchronised on the divided clock, and a divider that waits for it never starts. The offload test also drives the divider's **runtime path**: between the two power cycles of a target whose domain has a divider, with the group isolated and in reset, the firmware doubles `<domain>_clk_div_value` (`[TEST_PROGRESS]` reports "domain divider reprogrammed"), runs the second cycle at the new ratio and restores the default afterwards - the register-to-`clk_int_div` handshake that no other test exercises.
...
The block always carries **`tb_phase`**, the test-progress mailbox: `code[7:0]` and `seq[15:8]`, written by the generated test firmware at every phase transition and watched by the testbench, which prints `[TEST_PROGRESS]` at the write (testbench guide, section 6). On a NoC with `narrow_reduction` enabled the block also carries **`collective_ctrl`**: two 4-bit fields per multicast-target component, `<component>_coll_col_op` and `<component>_coll_row_op` (reset `7`, IntAdd), the operation the component's tiles stamp on their column and row reduction windows. It is described with the collectives in section 2.2 (Topology).
...
*   `fetch_enable` / `boot_enable`: Boolean. Generates a core wake-up register, **one bit per instance** (a mesh array gets a field as wide as its instance count, a single instance a scalar), connected to each tile's `fetch_en_i` / `en_sa_boot_i`.
*   `boot_addr`: Int/Hex. Generates a programmable boot address register, one per component: every instance of an array boots the same image.
*   `debug_req`: Boolean. Generates a programmable debug request register, one bit per instance.
...
*   `has_busy_status` / `has_eoc_status`: Boolean. Exposes status flags to software, one bit per instance. An `eoc_o` that is also named as an interrupt source keeps feeding the status register through its interrupt wire.
...
**On the mesh** the same declarations route between tiles: the generator declares one `intr_<component>_<port>` wire per interrupt source in the SoC top, assembles the dictionary-mapped vectors there and connects each tile's ports, exactly as on the crossbar (the two tops share one implementation, `core/interrupt_routing.py`). A source on a component that expands into several instances is an **array over the instances** (`intr_pulp_cluster_eoc_o[3:0]` for a 2x2 box), each tile driving its own slice, so `pulp_cluster.eoc_o` in a source expression names the whole array and `pulp_cluster.eoc_o[2]` one instance. The host declares its aggregator as on the crossbar (`intr_ext_i` with a dictionary source) and the tile passes the port through; in both topologies the vector takes the width the host isle is built for (the default of its `NumIntrsIn` parameter, for which the host's PLIC is regenerated per project - environment guide, section 7), so every routed line has a PLIC source and the generation refuses a bit beyond it; a source port is exported from its tile the moment any component names it, so a mailbox's `snd_irq_o` needs no declaration of its own. `noc_isle` routes one mailbox line to the host and another to its PULP array.
...
A `control_wire` component placed as a box is driven as an array too, through the SoC-side wires: the firmware writes every instance's boot-address registers through that instance's window, releases them all with one write of the fetch-enable field (one bit per instance), waits for every EoC bit and reads each instance's return register. The payload is built once, with instance 0's addresses, and relocates its control-unit accesses at run time: the instance ordinal sits in the cores' `mhartid` (the isle declares how, `OffloadHartBase` and `OffloadHartInstStride`) and multiplies the window stride, because a PULP cluster decodes its peripheral region against the cluster id while its local memory is the same canonical window for every cluster. The power-cycle and selective-power phases run on such a target exactly as on a memory-mapped one.

**Offload windows stay out of the host's cached regions.** Every contract has the host read what the target or the network wrote - return slots and registers, collective landings, the meta words it initialised itself - and CVA6 caches its CIE window (`Cva6ExtCieOnTop` / `Cva6ExtCieLength`, anchored under `0x8000_0000` or at `0x2000_0000`) and its LLC-out window without ever invalidating a line behind the core. An offload target inside them polls a stale copy, or reads back its own initialisation once a line is cached: generation refuses the window with both ranges in the message. `noc_isle` keeps both clusters below its CIE window, the Snitch box at `0x2100_0000` and the PULP array at `0x1800_0000`.

**Interrupt route witness.** When the host's `intr_ext_i` maps a bit to `<source>.<port>[k]` of an isle that publishes the `IrqSource*` contract (how a program enables, sets and clears line k through the isle's own registers; `mailbox_isle` does, `docs/hw/component_standardization.md` section 3) and the host isle publishes its PLIC contract (`Host*` localparams, section 5.5 there), the generated test drives that route end to end before the offload phases, without an interrupt handler: it raises line k through the source's registers, expects PLIC source `ext_irq_base + bit` pending, claims it under its own id, clears the line and expects it gone. The first such entry of the dictionary is taken. Every transcript records the outcome: the `[TB_IRQ]` monitor shows the source wire rising and falling, and the firmware reports the verdict as the `[TEST_PROGRESS]` phase "interrupt route to the PLIC verified" (a failure names the step on the UART and ends the test). The generation log names the route (`Interrupt route witness: mailbox line 7 -> manager.intr_ext_i[0] -> PLIC source 58`); a failure names the step. Nothing is emitted when either half is missing.

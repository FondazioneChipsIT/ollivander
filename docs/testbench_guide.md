...
4.  **Memory preload** (`$readmemh`, or a streamed SBA load inside the boot sequence under `preload_mode: jtag`) — see section 5.
...
5.  **Image load via SBA** (only with `preload_mode: jtag`, section 5): one streamed `sba_load` call per preload region, after the bring-up (the image may live in a gated tile) and before the handoff (the host must find it on wake-up).
...
The firmware image reaches the boot memory by one of two roads, chosen per project with `testbench.preload_mode`.

...
4.  **Memory preload** (`$readmemh`) — see section 5.
...
*   **JTAG driver stack** — a procedural replica of riscv-dbg's `jtag_test` driver, task by task, but without class-based timing constructs (which is what lets the same VIP compile under Verilator). Public tasks: `jtag_init()` (TAP reset, IDCODE check, debug module activation) and the System Bus Access family `sba_write32(addr, data)`, `sba_read32(addr, data)`, `sba_write32_verify(addr, data)` — memory-mapped accesses through the debug module, i.e. the same path an external debugger would use on silicon. Note for future work: this SBA path is the natural transport for architected firmware loading (wip chapter 2); today it carries the boot handoff only.
...
| `[TB_AXI_MON]` | generated AXI monitors | request/acceptance/data on watched ports (e.g. the boot memory's read port); on `force` projects this monitor also drives the force release |

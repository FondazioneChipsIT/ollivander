...
* **The object cache is version-checked**: Verilator's makefiles route every compile through `ccache` when one is on `PATH`, and they use precompiled headers — a combination that **ccache 3.x mishandles**, serving objects compiled against a stale header: the resulting model segfaults in its constructor before printing a line, deterministically, across rebuilds that wipe the work directory. The build therefore reads the cache's major version and, below 4, takes the precompiled headers off the compile lines, printing one line that says so. The measured cost of building without them is nil (6m26s against 6m43s on `noc`). ccache 4.x keeps them on: its changelog cites pch-handling fixes, and repeated runs never reproduced the issue there.

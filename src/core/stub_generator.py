...
def resolve_active_dependencies(files, seeds, outdir_path, global_options, fast_check_tool="questa"):
    ...
    # Per tool: the stub SET is tool-dependent (see the padframe rule below), so a
    # shared directory meant the second backend to run silently replaced the
    # first's stubs while its flist still pointed at them.
    stubs_dir = outdir_path / ".stubs" / fast_check_tool
    if stubs_dir.is_dir():
...
def generate_stubs(outdir_path: Path, soc_config, env_dependencies, base_dir: Path, fast_check_tool: str = "questa"):
    ...
    Not to be confused with src/tools/gen_vhdl_stubs.py, which also emits blackbox
    stubs and does a wholly different job. That one exists because Verilator
    cannot read VHDL AT ALL, so it parses VHDL entity declarations with a parser
    of its own and feeds the full Verilator SIMULATION build; this one reads
    SystemVerilog with pyslang to make FAST-CHECK cheap by not compiling the IPs.
    A stub also means something different on each side: those tie their outputs
    low so real traffic through a stubbed block fails loudly, while these preserve
    the signature and leave outputs open, being elaborated structurally and never
    run. Outputs land in different places too: .stubs/<tool>/ here, vhdl_stubs/
    there.

    The stub set is written PER TOOL because the policy is tool-dependent (the
    padframe components are compiled whole for Questa and stubbed for Verilator):
    a shared directory used to leave whichever backend ran first pointing at the
    other one's stubs.
    """
    ...
    def decompose_struct_params(stubs_content):
        ...
        if fast_check_tool == "verilator":
            ...
            #
            # FLATTEN ONLY WHAT CAN BE FLATTENED WHOLE. Rewriting the declaration to
            # 'int' is valid only if EVERY '<param>.<field>' access can be rewritten
            # to that field's literal value; the fields come from parsing the
            # struct's initializer, which yields nothing for a POSITIONAL '{...}
            # literal (tc_pad_pkg's default_tc_pad_config is one, its field names
            # living in trailing comments). Replacing the declaration anyway left
            # 'tc_pad_config.n_int' as a member access on an int - invalid code that
            # Verilator's own stub path tolerated and a strict front-end did not.
            # Skipping is safe: the parameter keeps its declared type.
            decl_replacements.sort(key=lambda x: x[0], reverse=True)
            for start, end, p_name, p_default in decl_replacements:
                known_fields = set(global_struct_fields[p_default])
                accessed_fields = set(re.findall(
                    r'\b' + re.escape(p_name) + r'\s*\.\s*([a-zA-Z_0-9]+)\b', stubs_content))
                if not accessed_fields <= known_fields:
                    print(f"  -> Keeping struct parameter '{p_name}' as declared: "
                          f"{', '.join(sorted(accessed_fields - known_fields))} "
                          f"{'is' if len(accessed_fields - known_fields) == 1 else 'are'} "
                          f"accessed but absent from '{p_default}' (positional initializer?)")
                    continue
    ...
    stubs_dir.mkdir(parents=True, exist_ok=True)   # '.stubs/<tool>': two levels now
...
"""
                ...
                if (p_clean.endswith('.sv') or p_clean.endswith('.v')) and not p.startswith('+'):
                    ...
                    if is_fast_compile_target(p_clean, outdir_path, fast_check_tool):
                        ...
                        if p_resolved_abs in stub_file_mapping:
                            if p_resolved_abs in active_files or 'generated/' in p_clean or 'components/infrastructure/' in p_clean:
                                stub_rel = f'"$ROOT/generated/.stubs/{fast_check_tool}/{stub_file_mapping[p_resolved_abs]}"'
                                new_tokens.append(stub_rel)
                                verilator_files.append(f'generated/.stubs/{fast_check_tool}/{stub_file_mapping[p_resolved_abs]}')
                                    ...
                                    if bp_resolved_abs in stub_file_mapping:
                                        if bp_resolved_abs in active_files or 'generated/' in bp_resolved or 'components/infrastructure/' in bp_resolved:
                                            ...
                                            try:
                                                ...
                                                verilator_files.append(f'generated/.stubs/{fast_check_tool}/{stub_file_mapping[bp_resolved_abs]}')
                                            except Exception as e:
    ...
    fast_tcl.append(f'vlog -suppress 13314,13233 {opts_str} -sv {outdir_path}/.stubs/{fast_check_tool}/{project_name}_stubs.sv')
    for stub in fast_check_stubs:
        fast_tcl.append(f'vlog -suppress 13314,13233 {opts_str} -sv {outdir_path}/.stubs/{fast_check_tool}/{stub["name"]}')
    ...
    # Add stubs
    verilator_f.append(f'{outdir_path.resolve()}/.stubs/{fast_check_tool}/{project_name}_stubs.sv')
    for stub in fast_check_stubs:
        verilator_f.append(f'{outdir_path.resolve()}/.stubs/{fast_check_tool}/{stub["name"]}')

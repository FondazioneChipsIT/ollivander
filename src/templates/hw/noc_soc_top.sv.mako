<%
  ...
  # Interrupt routing, shared with the crossbar top (core/interrupt_routing.py and the
  # defs imported below): the mesh had registers and firmware helpers for 'interrupts'
  # but no wires, so every routed line ended in a tile port tied to zero.
...
% if ir.assignments:
  // IR-level continuous assignments: a port claimed both by an exported interface and
  // by an interrupt route drives its 'intr_*' wire, and the exported top-level signal
  // is fed from that wire here (the crossbar top's rule, rtl_ir_builder.build_noc_ir).

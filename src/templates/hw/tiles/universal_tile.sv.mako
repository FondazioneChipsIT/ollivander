...
% else:
    .RouteAlgo     ( ${p_name}_soc_pkg::RouteCfgNoMcast.RouteAlgo ),
% endif
...
% else:
    .RouteCfg            ( ${p_name}_soc_pkg::RouteCfgNoMcast ),
% endif
    .AtopSupport         ( 1'b1 ),
    .WideRwDecouple      ( floo_${p_name}_noc_pkg::WideRwDecouple ),
    .VcImpl              ( floo_${p_name}_noc_pkg::VcImpl ),
...
% if use_join:
  <%
    c_data_width = None
    if 'AxiDataWidth' in valid_user_params:
    ...
    if c_data_width is None:
        if is_host and original_type == 'cheshire_isle':
            ...
            if use_join or has_slave_wide or has_master_wide:
                c_data_width = 512
            else:
                c_data_width = 64
        else:
  ...
  localparam floo_pkg::axi_cfg_t AxiCfgJoin = '{
    ...
    InIdWidth:  0,
    OutIdWidth: floo_pkg::max(floo_${p_name}_noc_pkg::AxiCfgN.OutIdWidth, floo_${p_name}_noc_pkg::AxiCfgW.OutIdWidth)
  };
  % else:
